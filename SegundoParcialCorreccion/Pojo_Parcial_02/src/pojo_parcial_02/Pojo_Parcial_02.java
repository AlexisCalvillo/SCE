...
public class Pojo_Parcial_02 
{
    ...
    public static void main(String[] args)
    {
       ...
       */
       ...
       String strNombre[] = {"Arcadio","Calvillo","Madrid"};
       ...
       custnew.setEmail("acalvillm@itam.mx"); // poner su correo del itam
       ...
       List<wscustord.CustomerOrder> enc = findAll_CustomerOrder();
       System.out.println("Lista de Encabezados de Pedidos:" + ( enc.isEmpty() ? " Lista sin elementos":"" ));
       for(wscustord.CustomerOrder co : enc){
            System.out.println("CustomerOrder:" +
                                " Id:" + co.getId() + 
                                " Fecha:"     + co.getDateCreated() +
                                " Clte:"     + co.getCustomerId().getId() +
                                ...
                                " Num. conf:" + co.getConfirmationNumber() +
                                " Monto:"    + co.getAmount());
            java.util.List<wsordprod.OrderedProduct> ordProdList = findOrderedProductByCustId(co.getId());
            for(wsordprod.OrderedProduct op: ordProdList ){
                System.out.println("Item: " + op.getProduct().getName() + 
                         " id: " + op.getProduct().getId()+
                         " Cantidad: " + op.getQuantity());
            }
       }
       //
       ...
       //Monto total de todos los pedidos
        System.out.println("El total de los pedidos es: " + totAmount() );
       //Monto total de todos los pedidos de un cliente
       
        for(wscust.Customer cust: liscust){
            System.out.println("Total por Cliente: " + cust.getName()+
                    " Id del cliente: " + cust.getId()+ 
                    " Total: " + totAmountForCustOrd(cust.getId()));
        }
       //
       // ======================  FIN del main() ===========================
       //     
    ...
    private static int count_CustomerOrder() {
        wscustord.WsPedidoHeader_Service service = new wscustord.WsPedidoHeader_Service();
        wscustord.WsPedidoHeader port = service.getWsPedidoHeaderPort();
        return port.count();
    }
    ...
    private static int count_OrderedProduct() {
        wsordprod.WSPedidoItem_Service service = new wsordprod.WSPedidoItem_Service();
        wsordprod.WSPedidoItem port = service.getWSPedidoItemPort();
        return port.count();
    }
    ...
    private static List<OrderedProduct> find_OrderedProductById(int id) {
        wsordprod.WSPedidoItem_Service service = new wsordprod.WSPedidoItem_Service();
        wsordprod.WSPedidoItem port = service.getWSPedidoItemPort();
        return port.findOrderedProductByCustId(id);
    }

    private static CustomerOrder find_CustomerOrderById(int id) {
        wscustord.WsPedidoHeader_Service service = new wscustord.WsPedidoHeader_Service();
        wscustord.WsPedidoHeader port = service.getWsPedidoHeaderPort();
        return port.findById(id);
    }

    private static java.util.List<wsordprod.OrderedProduct> findOrderedProductByCustId(int id) {
        wsordprod.WSPedidoItem_Service service = new wsordprod.WSPedidoItem_Service();
        wsordprod.WSPedidoItem port = service.getWSPedidoItemPort();
        return port.findOrderedProductByCustId(id);
    }
    ...
    private static double totAmount() {
        wscustord.WsPedidoHeader_Service service = new wscustord.WsPedidoHeader_Service();
        wscustord.WsPedidoHeader port = service.getWsPedidoHeaderPort();
        return port.totAmount();
    }

    private static double totAmountForCustOrd(int custId) {
        wscustord.WsPedidoHeader_Service service = new wscustord.WsPedidoHeader_Service();
        wscustord.WsPedidoHeader port = service.getWsPedidoHeaderPort();
        return port.totAmountForCustOrd(custId);
    }
    
    //Comentarios sobre las correcciones
    /*
    No tenía la BD correcta, estaba usando otra versión y, por no revisar, me atoré. 
    No sé si hay un problema con los agregados de tipo decimal de derby (al menos, no me dejó con un "createQuery" normal
    sino con un "nativequery" que sería saltarse la referencia del EJB de entidad. El SUM me
    marca un "must be numeric" con el createQuery
    */
